...
class Gallery(InlineUnit):
    async def gallery(
        ...
        *,
        ...
        :param next_handler: Callback function, which must return url for next
                             photo or list with photo urls
        :param custom_buttons: Custom buttons to add above native ones
        :param force_me: Either this gallery buttons must be pressed only by
                         owner scope or no
        :param always_allow: Users, that are allowed to press buttons in
                             addition to previous rules
        :param ttl: Time, when the gallery is going to be unloaded
        :param on_unload: Callback, called when gallery is unloaded and/or closed
        :param preload: Either to preload gallery photos beforehand or no.
                        If yes - specify threshold to be loaded
        :param gif: Whether the gallery will be filled with gifs
        :param manual_security: By default, security of inline buttons is
                                inherited from the caller (command)
        :param disable_security: Disable all security checks on this gallery
        :param silent: Whether the gallery must be sent silently
        :return: If gallery is sent, returns :obj:`InlineMessage`, otherwise
                 returns `False`
        """
        ...
        try:
            ...
            if _reattempt:
                ...
                del self._units[unit_id]
                await answer(
                    "🚫 <b>Can't send gallery. Error in logs</b>"
                    + (
                        "\n<i>"
                        + utils.escape_html(
