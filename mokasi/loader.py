...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        try:
            ...
            # Event loop is closed (e.g. interpreter shutdown)
            return None
...
def tag(*tags, **kwarg_tags):
    ...
    Currently available tags:
        • `no_commands` - Ignore all commands in watcher
        • `only_commands` - Capture only commands in watcher
        • `only_messages` - Capture only messages
    ...
    @loader.tag("no_commands")
    @loader.tag("no_commands", only_pm=True)
    @loader.tag(only_messages=True)
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?mokasi$", from_id=0000000000)

    💡 These tags can be used directly in `@loader.watcher`:
    @loader.watcher("no_commands", only_pm=True)
    """
...
def command(*args, **kwargs):
    """
    Decorator that marks function as bot command
    """
...
class Modules:
    ...
    def __init__(
        ...
        self._junk_collector_task = self._ensure_future(self._junk_collector())
        self.security = security.SecurityManager(client, db)
        self.inline = InlineManager(self.client, self._db, self, self.security)
        self.client.mokasi_loader = self
        self.client.mokasi_inline = self.inline

    @staticmethod
    def _ensure_future(coro) -> typing.Optional[asyncio.Future]:
        """Schedule a task, if there is a running event loop"""
        try:
    ...
    async def _start_junk_collector(self):
        """Start the junk collector, if it wasn't started at init time"""
        if not self._junk_collector_task:
            self._junk_collector_task = asyncio.ensure_future(self._junk_collector())

    async def _junk_collector(self):
        """
        Periodically reloads commands, inline handlers, callback handlers
        and watchers from loaded modules to prevent zombie handlers
        """
        while True:
            ...
            for module in self.modules:
                ...
                watchers.extend(module.watchers.values())
    ...
    def register_watchers(self, instance: Module):
        ...
        for _watcher in instance.watchers.values():
            self.watchers += [_watcher]

    def lookup(
        self,
        modname: str,
    ) -> typing.Union[bool, Module]:
        return next(
            (
                mod
    ...
    async def send_ready(self):
        ...
        await self._start_junk_collector()
